...
export default function DashboardTeamPage() {
  ...
  const goToNextStep = () => {
    if (!formData.displayName || !formData.email || !formData.branchLocation) {
      alert("Please complete the core leadership info before continuing.");
      return;
    }
    setFormStep((previous) => Math.min(previous + 1, 3));
  };

  const goToPreviousStep = () => {
    setFormStep((previous) => Math.max(previous - 1, 1));
  };
          ...
          <div className={styles.container}>
                ...
                <div className={styles.editorHeader}>
                  <div>
                    <p className={styles.kicker}>{creatingMember ? "Create Leadership" : "Edit Leadership"}</p>
                      <h2>{creatingMember ? "Add a new leader" : editingMember?.displayName}</h2>
                    <p className={styles.description}>{creatingMember ? "Create a leadership account and save it directly to Firestore." : "Update branch details and profile information for this leader."}</p>
                    <div className={styles.stepper}>
                      <div className={`${styles.stepItem} ${formStep === 1 ? styles.stepActive : ""}`}>1. Basics</div>
                      <div className={`${styles.stepItem} ${formStep === 2 ? styles.stepActive : ""}`}>2. Branch & media</div>
                      <div className={`${styles.stepItem} ${formStep === 3 ? styles.stepActive : ""}`}>3. Review</div>
                    </div>
                  </div>
                ...
                <div className={styles.editorCard}>
                  <form className={styles.formGrid} onSubmit={handleFormSubmit}>
                    ...
                    <div className={styles.formActions}>
                      {formStep > 1 ? (
                        <button type="button" className={styles.cancelBtn} onClick={goToPreviousStep}>
                          Back
                        </button>
                      ) : (
                        <button type="button" className={styles.cancelBtn} onClick={closeEditor}>
                      ...
                      <button type="submit" className={styles.saveBtn} disabled={saving}>
                        {formStep === 3 ? (saving ? "Saving..." : creatingMember ? "Create Leadership" : "Update Leadership") : "Continue"}
                      </button>
                    </div>
                  </form>
                </div>
            ...
            {teamMembers.length === 0 ? (
              <div className={styles.emptyState}>
                ...
                <p>Add the first leadership account to get started.</p>
              </div>
            ) : (
